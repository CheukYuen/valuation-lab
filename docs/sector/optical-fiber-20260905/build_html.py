...
def inline(text):
 ...
 text=re.sub(r"\b([JGNBMTFCUEDH])(?= p[0-9])",lambda m:f'<a href="#p0-source-{m[1]}">{m[1]}</a>',text)
 return text

lines=(BASE/'REPORT.md').read_text().splitlines();parts=[];toc=[];i=0;opened=False;section=0
section_total=sum(x.startswith('## ') for x in lines)
chart_total=sum(x.startswith(('<!-- chart:', '<!-- p0chart:', '<!-- p0timeline')) for x in lines)
broker_section=False
while i<len(lines):
 line=lines[i];i+=1
 if line.startswith('<!-- p0table:'):
  parts.append(p0.table(line.split(':',1)[1].split(' -->')[0]));continue
 if line.startswith('<!-- p0chart:'):
  parts.append(p0.chart(line.split(':',1)[1].split(' -->')[0]));continue
 if line.startswith('<!-- p0calc:'):
  parts.append(p0.calc(line.split(':',1)[1].split(' -->')[0]));continue
 if line.startswith('<!-- p0timeline'):
  parts.append(p0.timeline());continue
 ...
 if line.startswith('## '):
  ...
  heading=re.sub(r'^\d+\s+', '',line[3:]);broker_section='投行研报数据与分歧' in heading;section+=1;anchor=f'section-{section}';toc.append((anchor,heading))
  parts.append(f'<section id="{anchor}" aria-labelledby="title-{section}"><div class="section-kicker">{section:02d} / {section_total:02d}</div><h2 id="title-{section}">{inline(heading)}</h2>');opened=True;continue
 ...
 if line.startswith('|'):
  ...
  isbroker=broker_section
  if isbroker and '原评级' in rows[0]:parts.append('<div class="broker-tools" aria-label="筛选投行报告"><span>公司筛选</span><button class="selected" data-filter="全部" aria-pressed="true">全部</button><button data-filter="长飞光纤" aria-pressed="false">长飞光纤</button><button data-filter="中天科技" aria-pressed="false">中天科技</button><span id="broker-count" role="status">原估值报告</span></div>')
...
nav+='<a href="#p0-evidence">P0研报证据</a><a href="#full-data">完整数据</a><a href="#calculations">计算过程</a><a href="#sources">来源与缺口</a>'
css='''
...
'''
js='''
document.querySelectorAll('[data-filter]').forEach(button=>button.addEventListener('click',()=>{const selected=button.dataset.filter;document.querySelectorAll('[data-filter]').forEach(b=>{b.classList.toggle('selected',b===button);b.setAttribute('aria-pressed',String(b===button));});document.querySelectorAll('.broker-table tbody tr').forEach(row=>row.hidden=selected!=='全部'&&row.dataset.company!==selected);document.getElementById('broker-count').textContent=document.querySelector('.broker-table tbody').querySelectorAll('tr:not([hidden])').length+' 份报告';}));
...
'''
css+=CSS+palette_css()+p0.CSS
js+=(BASE/'html_audit.js').read_text()+p0.JS
target='index.html'
parts.append(p0.appendix())
parts.append(appendix())
page='''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="光纤光缆与AI光连接行业概览，长飞与中天主线，Wind、Choice和投行研报交叉研究。"><title>光纤光缆与 AI 光连接行业概览</title><style>'''+css+'''</style></head><body><header class="topbar"><div class="brand">VALUATION LAB / 行业研究</div><div class="topbar-actions"><a class="desktop-only" href="#sources">来源与口径</a><a href="#full-data">完整数据</a><button class="print" id="print-report">打印</button></div></header><div class="reading-progress" aria-hidden="true"></div><div class="layout"><aside class="sidebar"><p class="sidebar-title">CONTENTS / 研究目录</p><nav class="toc" aria-label="章节目录">'''+nav+'''</nav><div class="sidebar-note">各家投行保留原报告日期。所有目标价均为原报告观点，未平均为本研究的目标价。<a href="#calculations">计算过程</a><a href="#sources">查询记录与证据边界 ↗</a></div></aside><main><div class="hero"><div class="eyebrow">SECTOR OVERVIEW / 2026.09</div><h1>光纤光缆行业研究<br>高价能持续多久</h1><p class="deck">从分规格价格、终端需求和扩产时序，追踪利润如何传导至长飞与中天。将已观察到的改善，与尚待兑现的长期假设分开。</p><div class="meta"><span>研究日期 2026 年 9 月 5 日</span><span>估值截点 2026 年 9 月 2 日</span><span>财务基准 2025A / 2026H1</span><span>Codex 整理 · 内部研究</span></div><div class="keyline"><div class="metric"><div class="value">量 · 价 · 供给</div><small>先判断景气持续时间</small></div><div class="metric"><div class="value">成本 · 利润</div><small>再判断收益落在哪个环节</small></div><div class="metric"><div class="value">来源可追溯</div><small>研报原文、分歧与核验回执</small></div></div><div class="evidence-note"><strong>证据边界</strong> 价格、供给、需求、成本与公司利润桥已逐项整理。有效供给、细分需求及完整量价权重仍有缺口，未据此生成精确供需平衡；投行预测与原始披露分开。</div></div><details class="mobile-toc"><summary>展开研究目录</summary>'''+nav+'''</details>'''+''.join(parts)+'''<footer class="footer">研究文件截至 2026 年 9 月 5 日。市场数值冻结于 9 月 2 日。SVG 图表、明细、公式和文本回执均已嵌入本页，可离线阅读；原始研究档案继续保留。</footer></main></div><script>'''+js+'''</script></body></html>'''
page=re.sub(r'<a href="[^"]*\.xlsx"[^>]*>.*?</a>', '<a href="#full-data">完整数据（已迁移）</a>', page)
(BASE/target).write_text(page)
assert page.count('<section id=')==section_total+4
assert page.count('<svg ')==chart_total
